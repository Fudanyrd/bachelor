...
We also classified the collected datasets/benchmarks by their programming
languages. We found that most datasets consist of no more than
two programming languages, 
except one multi-lingual dataset (XLCoST \citep{xlcost}),
The result of our classification is shown in Figure~\ref{fig:lang}.
We concluded that today's main-stream language, \texttt{C}, \texttt{C++},
\texttt{Python}, and \texttt{Java} accounts for more than 90\% of studied
datasets, if we assume that the two Android-repository dataset 
(\citep{kong2019mining, androiddataset}) is developed with \texttt{Java}.
...
\begin{figure}[ht]
	...
	"Android" means applications developed for Android OS, which are mostly written in Java. }
	\label{fig:lang}
...
\begin{itemize}
	\item We categorized collected 25 bug datasets/benchmarks
	by the data source, and found that most (56\% of) dataset are compiled
	from public code repositories (as shown in Figure~\ref{fig:data}).
	...
	\item We found by auditing the distribution of programming language that
	existing program fix datasets/benchmarks lack support for 
	newer programming languages (e.g. \texttt{TypeScript}, \texttt{Golang}, \texttt{Rust}).
	This may become possible directions of future bug datasets.
\end{itemize}
